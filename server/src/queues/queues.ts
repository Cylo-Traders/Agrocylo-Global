...
export function getGroupOrderExpiryQueue(): any {
  if (!groupOrderExpiryQueue)
    groupOrderExpiryQueue = new Queue('group-order-expiry', { connection: getConnection() });
  return groupOrderExpiryQueue;
}

const PRICE_INDEX_INTERVAL_MS = 60 * 60 * 1000;
const RECONCILIATION_INTERVAL_MS = 15 * 60 * 1000; // 15 minutes
const GROUP_ORDER_EXPIRY_INTERVAL_MS = 60 * 1000;
...
export async function scheduleGroupOrderExpiry(): Promise<void> {
  await getGroupOrderExpiryQueue().add(
    'expire-group-orders',
    {},
    {
      repeat: { every: GROUP_ORDER_EXPIRY_INTERVAL_MS },
      jobId: 'group-order-expiry',
      removeOnComplete: 10,
      removeOnFail: 10,
    }
  );
