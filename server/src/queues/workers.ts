...
import { expireGroupOrders, finalizeReadyGroupOrders } from '../services/groupOrderService.js';
...
export function startWorkers(): RunningWorkers {
  ...
  const indexing = new Worker(
    'indexing',
    (job: any) => withJobContext('indexing', String(job.id), job.name, () => processIndexing(job)),
    opts
  );
  ...
  const reconciliation = new Worker(
    'reconciliation',
    (job: any) =>
      withJobContext('reconciliation', String(job.id), job.name, () => processReconciliation(job)),
    { ...opts, concurrency: 1 }
  );
  const groupOrderExpiry = new Worker(
    'group-order-expiry',
    (job: any) =>
      withJobContext('group-order-expiry', String(job.id), job.name, async () => {
        await finalizeReadyGroupOrders();
        await expireGroupOrders();
      }),
    { ...opts, concurrency: 1 }
  );

  const workers = [indexing, analytics, notifications, reconciliation, groupOrderExpiry];
  ...
  const events = [
    ...
    new QueueEvents('group-order-expiry', { connection }),
  ];
