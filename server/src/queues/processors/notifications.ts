...
import type { SendEmailJobData, SendPushJobData, SendWebSocketJobData } from '../job-types.js';
...
async function handleSendPush(job: Job<SendPushJobData>): Promise<void> {
  ...
  try {
    ...
    logger.info('Push notification published for API WebSocket delivery', {
      jobId: job.id,
      walletAddress,
    });
...
async function handleSendWebSocket(job: Job<SendWebSocketJobData>): Promise<void> {
  ...
  try {
    if (wallets && wallets.length > 0) {
      await publishWsEvent({ event, data, wallets });
      ...
      await publishWsEvent({ event, data });
