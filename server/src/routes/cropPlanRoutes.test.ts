...
describe('CropPlan routes (Issue #658)', () => {
  ...
  it('POST /crop-plans creates a crop plan when valid dates are provided', async () => {
    ...
    const res = await request(app).post('/crop-plans').send({
      farmerWallet: 'G1234567890',
      cropName: 'Cassava',
  ...
  it('GET /crop-plans/aggregate returns non-PII aggregated harvest volume for buyers', async () => {
    ...
    const res = await request(app).get('/crop-plans/aggregate?region=North&month=8&year=2026');
  ...
  it('POST /crop-plans/:id/reminder triggers harvest reminder when N days before harvest', async () => {
    ...
    vi.mocked(prisma.cropPlan.updateMany).mockResolvedValue({ count: 1 } as any);
    ...
    expect(prisma.cropPlan.updateMany).toHaveBeenCalledWith({
      where: { id: 'cp-123', reminderSent: false },
      data: { reminderSent: true },
    });
    expect(res.body.reminderStatus).toBe('sent');
  });
});
