...
// @ts-expect-error - graphql-depth-limit has no types
...
function depthForSelectionSet(
  ...
  for (const sel of selectionSet.selections) {
    if (sel.kind === Kind.FIELD) {
      ...
      const childDepth = depthForSelectionSet(
        sel.selectionSet,
        fragments,
        currentDepth + 1,
        visitedFragments
...
function enforceLimits(query: string, document: DocumentNode): void {
  if (query.length > MAX_QUERY_SIZE) {
    throw new ApiError(
      400,
      'Bad Request',
      `Query too large: ${query.length} > ${MAX_QUERY_SIZE} characters`
    );
  }
  ...
  if (aliasCount > MAX_ALIAS_COUNT) {
    throw new ApiError(
      400,
      'Bad Request',
      `Alias count ${aliasCount} exceeds limit of ${MAX_ALIAS_COUNT}`
    );
  }
  if (fieldCount > MAX_FIELD_COUNT) {
    throw new ApiError(
      400,
      'Bad Request',
      `Field count ${fieldCount} exceeds limit of ${MAX_FIELD_COUNT}`
    );
  }
}
...
export async function executeGraphQL(
  ...
  try {
    ...
    throw new ApiError(
      400,
      'Bad Request',
      e instanceof Error ? e.message : 'Invalid GraphQL query'
    );
  }
  ...
  const rootValue: Record<string, any> = {
    ...
    orders: async (args: Record<string, unknown>) => {
      const requestedRaw = (args.wallet ??
        args.walletAddress ??
        args.wallet_address ??
        args.id) as unknown;
