...
describe('GraphQL Gateway — IDOR protection', () => {
  ...
  it('matrix via variables: every wallet arg variant denied', async () => {
    ...
    for (const c of cases) {
      ...
      expect(
        result.errors,
        `field ${c.field} arg ${c.argName} via variables should be denied`
      ).toBeDefined();
    }
  });
});
...
describe('GraphQL Gateway — DoS protections', () => {
  ...
  it('depth bomb via fragment chain is rejected with 400', async () => {
    const fragmentDepthQuery = `
      ...
      query { ...F9 }
    `;
    await expect(executeGraphQL(fragmentDepthQuery, {}, WALLET_A)).rejects.toMatchObject({
      status: 400,
    });
  });
  ...
  it('alias explosion is rejected with 400', async () => {
    const aliases = Array.from(
      { length: MAX_ALIAS_COUNT + 5 },
      (_, i) => `alias${i}: product(id: "1")`
    ).join(' ');
    const query = `{ ${aliases} }`;
    await expect(executeGraphQL(query, {}, WALLET_A)).rejects.toMatchObject({ status: 400 });
  });
  ...
  it('allows shallow introspection but still depth-limits an introspection bomb', async () => {
    // buildSchema includes introspection, so a shallow query must succeed…
    const introspection = `{ __schema { queryType { name } } }`;
    const result = await executeGraphQL(introspection, {}, WALLET_A);
    expect(result.errors).toBeUndefined();
    expect(result.data.__schema).toBeDefined();

    // …while a deeply nested one stays a rejected DoS vector (MAX_DEPTH = 8,
    // this nests 9 levels).
    const deepIntrospection =
      '{ __schema { types { fields { type { fields { type { fields { type { name } } } } } } } } }';
    await expect(executeGraphQL(deepIntrospection, {}, WALLET_A)).rejects.toMatchObject({
      status: 400,
    });
  });
});
