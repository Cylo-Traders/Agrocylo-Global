...
import { createOrJoinGroupOrder, expireGroupOrders } from './groupOrderService.js';
...
beforeEach(() => {
  vi.clearAllMocks();
  prismaMock.groupOrder.updateMany.mockResolvedValue({ count: 1 });
});
...
describe('groupOrderService', () => {
  it('submits the batch when the threshold is reached', async () => {
    ...
    expect(prismaMock.groupOrder.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ id: 'go1', status: 'PENDING' }),
  ...
  it('expires stale pools and cancels reserved contributions', async () => {
  ...
  it('does not expire or notify a pool after another worker claimed it', async () => {
    prismaMock.groupOrder.findMany.mockResolvedValue([{ id: 'go1', contributions: [] }] as never);
    prismaMock.groupOrder.updateMany.mockResolvedValue({ count: 0 } as never);

    const expired = await expireGroupOrders(new Date('2026-07-27T00:00:00.000Z'));

    expect(expired).toEqual([]);
    expect(prismaMock.groupOrder.findUnique).not.toHaveBeenCalled();
    expect(NotificationService.notify).not.toHaveBeenCalled();
    expect(prismaMock.groupOrderContribution.updateMany).not.toHaveBeenCalled();
  });
});
