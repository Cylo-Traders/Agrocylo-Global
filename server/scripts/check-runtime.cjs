...
function validateToolchain(nodeVersion, currentNpmVersion) {
  ...
  if (!isSupportedNode(nodeVersion)) {
    errors.push(
      `Unsupported Node.js ${nodeVersion}; required ${NODE_RANGE}. Run \`nvm use\` from server/.`,
    );
  }
...
function main() {
  ...
  if (errors.length > 0) {
    console.error(["Server toolchain check failed:", ...errors.map((error) => `- ${error}`)].join("\n"));
    process.exitCode = 1;
    return;
  }

  console.log(
    `Server toolchain OK: Node.js ${process.versions.node}, npm ${currentNpmVersion}`,
  );
}
