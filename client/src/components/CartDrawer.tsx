...
export default function CartDrawer() {
  ...
  const totalsPerCurrency = useMemo(() => {
    const map = new Map<string, { gross: bigint; fee: bigint; net: bigint }>();
    for (const g of cart.groups) {
      const gross = BigInt(g.subtotal || "0");
      const fee = feeFromGrossStroops(gross);
      const net = gross - fee;
      const cur = g.currency;
      const existing = map.get(cur);
      if (!existing) {
        map.set(cur, { gross, fee, net });
      } else {
  ...
  async function createOrdersForCart(groups: CartGroup[]) {
    ...
    try {
      ...
      for (let i = 0; i < snapshotGroups.length; i++) {
        const group = snapshotGroups[i];
